...
MODEL_PATH = os.path.join(ARTIFACTS_DIR, "logistic_regression_model_binance.joblib")
SCALER_PATH = os.path.join(ARTIFACTS_DIR, "scaler_binance.joblib")

def train_binance_model():
    """
    Trains the logistic regression model and saves the artifacts.
    """
    # 1. Load Data
    raw_df = load_binance_data_to_dataframe()
    if raw_df.empty:
        print("Data loading failed. Exiting training process.")
        return

    # 2. Feature Engineering
    processed_df, scaler = feature_engineer_binance(raw_df)
